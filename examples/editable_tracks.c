...
static char *opt_cache_base_dir = "./cache/";
...
OsmGpsMapTrack* maptrack;

static GdkPixbuf *g_star_image = NULL;
static OsmGpsMapImage *g_last_image = NULL;

static int click_down_x = 0;
static int click_down_y = 0;
...
on_button_press_event (GtkWidget *widget, GdkEventButton *event, gpointer user_data)
{
    ...
    {
        GSList* points = osm_gps_map_track_get_points(maptrack);
        int ctr = 0;
        while(points)
        {
            int cx, cy;
            ...
            if(dist_sqrd <= (5*5))
            {
                osm_gps_map_track_remove_point(maptrack, ctr);
...
on_button_release_event (GtkWidget *widget, GdkEventButton *event, gpointer user_data)
{
    ...
    if((click_down_x == (int)event->x) && (click_down_y == (int)event->y))
    {
        ...
        osm_gps_map_track_add_point(maptrack, &coord);
    }
...
main (int argc, char **argv)
{
    ...
    maptrack = osm_gps_map_track_new();

    g_object_set(maptrack, "editable", TRUE, NULL);
    ...
    osm_gps_map_track_add(map, maptrack);
    ...
    g_signal_connect (G_OBJECT (maptrack), "changed", G_CALLBACK (on_track_changed), maptrack);
